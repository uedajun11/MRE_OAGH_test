...
class PDataset(Dataset):
    def __init__(self, dir_input, offsets, fov=0.2, extension=".pt", transform=None):
        self.dir_input = dir_input
        self.extension = extension
        self.offsets = offsets
        self.fov = float(fov)
...
def get_Pdataloader_for_train(dir_input, offsets=8, fov=0.2, batch_size=10, snr_db=None):
    #ids = [f[:-4] for f in os.listdir(dir_input)]
    #ids = [f[:-4] for f in os.listdir(dir_input) if os.path.isfile(os.path.join(dir_input, f))]
    transform = AddGaussianNoiseSNR(snr_db) if snr_db is not None else None
    dset = PDataset(dir_input, offsets, fov,transform=transform)
    dataloaders = {}
...
def get_Pdataloader_for_val(dir_input, offsets=8, fov=0.2, batch_size=10, snr_db=None):
    #ids = [f[:-4] for f in os.listdir(dir_input)]
    #ids = [f[:-4] for f in os.listdir(dir_input) if os.path.isfile(os.path.join(dir_input, f))]
    transform = AddGaussianNoiseSNR(snr_db) if snr_db is not None else None
    dset = PDataset(dir_input, offsets, fov,transform=transform)
